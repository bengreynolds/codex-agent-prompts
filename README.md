...
## Quick start
1. Pick a workflow from `prompts/workflows/` (organized by category).
...
These files are typically created at the start of a task (see `prompts/workflows/setup/branch-setup.md` and `prompts/workflows/context/agent-planning-review.md`). They are meant to be temporary working artifacts for a single task or branch session, not permanent documentation.
